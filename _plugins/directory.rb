...
# Modified by:  Amsul http://amsul.ca
# Description:  The directory tag lets you iterate over files at a particular path. If
#               files conform to the standard Jekyll format, YYYY-MM-DD-file-title, then
#               those attributes will be populated on the yielded file object. The
#               `forloop` object maintains [its usual context](http://wiki.shopify.com/UsingLiquid#For_loops).
#
# Syntax:
#
...
# - `name` - The basename
# - `date` - The date extracted from the filename, otherwise the file's modified time
# - `slug` - The basename with date and extension removed
# - `title` - The front-matter configured title, otherwise the titlecase'd slug
#
...
module Jekyll
  ...
  class DirectoryTag < Liquid::Block
    ...
    def process_filename(name)
      ...
      yaml_config = read_yaml(filepath[0...-1], filepath.pop)
