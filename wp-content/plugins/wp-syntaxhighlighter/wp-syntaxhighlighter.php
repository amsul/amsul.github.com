...
function wp_sh_escape_code($content) {
	$match_num = preg_match_all('|\[sourcecode[^\]]*?\].*?\[/sourcecode\]|is', $content, $match);
	if ($match_num != 0 || $match_num != false) {
		for ($i = 0; $i < $match_num; $i++) {
			$code = preg_replace('|\[sourcecode[^\]]*?\](.*?)\[/sourcecode\]|is', '$1', $match[0][$i]);
			if (strpos($code, "<") !== false || strpos($code, ">") !== false || strpos($code, '"') !== false || strpos($code, "'") !== false || preg_match('/&[^(lt;)|(gt;)|(amp;)|(quot;)|(#039;)]/i', $code)) {
				if (strpos($code, "<") === false && strpos($code, ">") === false && !preg_match('/&[^(lt;)|(gt;)|(amp;)|(quot;)|(#039;)]/i', $code)) {
					...
					$replaced_code = str_replace("'", '&#039;', $pre_replaced_code);
				} else {
			...
			$content = str_replace($code, $replaced_code, $content);
		}
	}
...
function wp_sh_wp_filter_kses($content) {
	...
	if ($match_num != 0 || $match_num != false) {
		for ($i = 0; $i < $match_num; $i++) {
			$code = preg_replace('|\[sourcecode[^\]]*?\](.*?)\[/sourcecode\]|is', '$1', $match[0][$i]);
			$replaced_code = preg_replace("/&amp;#([0-9A-Fa-f]{2,4});/i", "&lt;!--[$1]--&gt;", $code);
			$content = str_replace($code, $replaced_code, $content);
		}
	}
...
function wp_sh_replace_marker($content) {
	...
	if ($match_num != 0 || $match_num != false) {
		for ($i = 0; $i < $match_num; $i++) {
			$code = preg_replace('|<pre[^>]*?>(.*?)</pre>|is', '$1', $match[0][$i]);
			$replaced_code = str_replace('&lt;!--[/pre]--&gt;', '&lt;/pre&gt;', $code);
			$content = str_replace($code, $replaced_code, $content);
		}
	}
	return $content;
